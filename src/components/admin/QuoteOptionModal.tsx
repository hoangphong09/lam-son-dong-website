...
export const QuoteOptionModal: React.FC<QuoteOptionModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-in fade-in duration-200">
      <div className="bg-white border border-slate-200 w-full max-w-2xl max-h-[92vh] overflow-y-auto shadow-2xl text-slate-900 rounded relative">
        {/* Close button */}
        <button
          onClick={onClose}
          disabled={isSubmitting}
          className="absolute top-4 right-4 w-8 h-8 border border-slate-300 bg-slate-100 hover:bg-slate-200 text-slate-600 hover:text-slate-950 flex items-center justify-center transition-all z-10 rounded"
        >
          <X className="w-4 h-4" />
        </button>
        ...
        <div className="p-6 border-b border-slate-200 bg-slate-50">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-bold text-slate-900 uppercase tracking-tight font-['Plus_Jakarta_Sans']">
                {option ? 'Chỉnh Sửa Tùy Chọn Báo Giá' : 'Thêm Tùy Chọn Báo Giá Mới'}
              </h3>
              <p className="text-xs text-slate-600 font-normal mt-0.5">
                Cấu hình các mục tiêu, đơn giá vị trí và gói dịch vụ hiển thị trên form báo giá
              </p>
            </div>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          {errorMessage && (
            <div className="p-3 bg-rose-50 border border-rose-300 text-rose-800 text-xs rounded flex items-center gap-2">
              ...
              {(['target_objective', 'pricing_tier', 'service_type'] as QuoteOptionCategory[]).map((cat) => (
                <button
                  ...
                  className={`p-3 text-left border rounded text-xs transition-all ${
                    category === cat
                      ? 'border-[#c5a059] bg-amber-50/80 font-bold text-slate-950 shadow-xs ring-1 ring-[#c5a059]'
                      : 'border-slate-200 bg-slate-50 text-slate-600 hover:bg-slate-100'
                  }`}
                >
                  <div className="font-bold uppercase tracking-wider text-[11px]">
                    {cat === 'target_objective' && '1. Mục tiêu'}
                    {cat === 'pricing_tier' && '2. Ca trực / Đơn giá'}
                    {cat === 'service_type' && '3. Gói dịch vụ kèm'}
                  </div>
                  <div className="text-[10px] text-slate-500 font-normal mt-0.5 line-clamp-1">
                    ...
                    {cat === 'service_type' && 'Tiện ích / Trang bị'}
                  </div>
                </button>
              ))}
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2 bg-slate-50 border border-slate-300 focus:border-amber-600 focus:bg-white text-slate-900 text-xs rounded focus:outline-hidden"
              />
            </div>
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2 bg-slate-50 border border-slate-300 focus:border-amber-600 focus:bg-white text-slate-900 text-xs font-mono rounded focus:outline-hidden"
              />
            </div>
          </div>
          ...
          <div className="p-4 bg-slate-50 border border-slate-200 rounded">
            ...
            <div className="bg-white p-3 border border-slate-300 rounded shadow-xs flex items-center justify-between gap-3">
              <div>
                <div className="flex items-center gap-2">
                  <span className="font-bold text-xs text-slate-900">{label || 'Tiêu đề tùy chọn'}</span>
          ...
          <div className="pt-4 border-t border-slate-200 flex items-center justify-end gap-3">
            <button
              ...
              onClick={onClose}
              disabled={isSubmitting}
              className="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-bold uppercase tracking-wider rounded transition-colors"
            >
              Hủy bỏ
            </button>
            <button
              type="submit"
              disabled={isSubmitting}
              className="px-6 py-2 bg-[#c5a059] hover:bg-[#b8860b] text-slate-950 text-xs font-black uppercase tracking-wider flex items-center gap-2 rounded shadow transition-all disabled:opacity-50"
            >
